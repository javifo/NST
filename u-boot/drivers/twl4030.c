...
usb_state twl4030_get_vbus_status(void)
{
	u8 data;
	int stat;
	...
	stat = i2c_read(TWL4030_CHIP_PM_MASTER, PM_MASTER_STS_HW_CONDITIONS, 1, &data, 1);
	if (stat) {
		printf("i2c_read error in %s\n",__FUNCTION__);
	}
	data &= 0x84; /* BIT(7) | BIT(2) */

	if (data == 0)
		return USB_LINK_NONE;

	if (data & 0x04)
		return USB_LINK_ID;

	return USB_LINK_VBUS;
}
