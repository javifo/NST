...
static const char * const prod_string(u32 btype)
{
    switch (btype & 0x38) {
    case BOARD_ENCORE_PROD_WLAN:
        return "WLAN";
    case BOARD_ENCORE_PROD_3G:
        return "3G";
    default:
        return "Unknown";
    }
...
static inline int is_encore_3g(void) 
{
    ...
    return ((gd->bd->bi_board_revision & 0x38) == BOARD_ENCORE_PROD_3G);
}
...
int board_init(void)
{
    ...
    switch(pid) {
    case ENCORE_3G:
        gd->bd->bi_board_revision = BOARD_ENCORE_PROD_3G;
        MUX_VAL(CP(ETK_D7_ES2 ), (IDIS | PTD | DIS | M4)); /*GIO-21  MODEM_EN */
        MUX_VAL(CP(GPMC_A1), (IDIS | PTD | DIS | M4)); /*GIO-34  MODEM_nON NOT PRESENT */	
        MUX_VAL(CP(GPMC_D14), (IDIS | PTD | DIS | M4)); /*GPMC_D14 GPIO50 misc I/O*/
        MUX_VAL(CP(GPMC_D13), (IDIS  | PTD | DIS  | M4)); /* GPMC_D13 MODEM-USB-EN */
    	MUX_VAL(CP(ETK_D9_ES2 ), (IDIS  | PTD | DIS  | M4)) /* GIO-23  HSUSB2_RSTn */
        break;
    case ENCORE_WLAN:
    case UNKNOWN_PRODUCT:
        gd->bd->bi_board_revision = BOARD_ENCORE_PROD_WLAN;
    default:
        break;
    }
    ...
    switch(bid) {
        ...
        case PVT:
            ...
            break;
        default:   
            gd->bd->bi_board_revision |= BOARD_ENCORE_REV_UNKNOWN;
        }
...
int misc_init_r(void)
{
    ...
    printf("Hardware arch: %s %s rev: %s\n", 
            arch_string(gd->bd->bi_arch_number), 
            prod_string(gd->bd->bi_board_revision),
    ...
    if (is_encore_3g()) {
        ...
        gpio_pin_init(34, GPIO_OUTPUT, GPIO_LOW);
...
{
    // This is safe to enter with boxer since the btype will be unknown
    if (btype == BOARD_OMAP3621_EVT1A ) {
        if (get_board_rev()>=BOARD_ENCORE_REV_EVT2) {
        // Only EVT2 has special config, for now...
            MUX_UPDATE_EVT2();
        }
    }
}
