...
/* ========================================================================== */
/**
*  @fn  gpio_two_pin_init_write()    function_description
*
*  @see  gpio.h
*/
/* ========================================================================== */
int gpio_two_pin_init_write(u32 gpio_pin1, gpio_level_t gpio_pin1_level, u32 gpio_pin2, gpio_level_t gpio_pin2_level)
{
int        retVal = 0;
u32        gpioModule_p1,gpioModule_p2;
vu32       *gpioRegs[6] = {(vu32*)GPIO1_BASE, (vu32*)GPIO2_BASE,
                           (vu32*)GPIO3_BASE, (vu32*)GPIO4_BASE,
                           (vu32*)GPIO5_BASE, (vu32*)GPIO6_BASE};
vu32        pinStates, enableStates;

    if (gpio_pin1 > GPIOs_COUNT || gpio_pin2 > GPIOs_COUNT)
    {
        retVal = -1;
        goto EXIT;
    }

    gpioModule_p1 = gpio_pin1 >> 5;
    gpioModule_p2 = gpio_pin2 >> 5;

    if (gpioModule_p1 != gpioModule_p2) {
        retVal = -1;
        goto EXIT;
    }

    gpio_module_init(gpioModule_p1);    /* ensure gpio module is initialized */

    pinStates= gpioRegs[gpioModule_p1][GPIO_DATAOUT];
    pinStates &= ~(GPIO_MASK(gpio_pin1)|GPIO_MASK(gpio_pin2));
    if (gpio_pin1_level != GPIO_LOW) pinStates |= GPIO_MASK(gpio_pin1);
    if (gpio_pin2_level != GPIO_LOW) pinStates |= GPIO_MASK(gpio_pin2);
    gpioRegs[gpioModule_p1][GPIO_DATAOUT] = pinStates;

    enableStates = gpioRegs[gpioModule_p1][GPIO_OE];
    enableStates &= ~(GPIO_MASK(gpio_pin1)|GPIO_MASK(gpio_pin2));
    gpioRegs[gpioModule_p1][GPIO_OE] = enableStates;
EXIT:
    return retVal;
}
...
int do_gpio(cmd_tbl_t * cmdtp, int flag, int argc, char *argv[])
{
    ...
    case 5:
        ...
        if ((argv[2][0] == 'h') || (argv[2][0] == '1')) s1=1;
        pin2 = simple_strtoul(argv[3], NULL, 10);
        if ((argv[4][0] == 'h') || (argv[4][0] == '1')) s2=1;
        rcode = gpio_two_pin_init_write(pin, s1, pin2, s2);
        if (rcode < 0)
        {
            printf("bank not same or invalid pin number\n");
        }
        break;
	case 3:			/* set pin */
		pin = simple_strtoul(argv[1], NULL, 10);
...
U_BOOT_CMD(
	...
	"num [in|low|high|0|1]\n"
    "num1 high|low|0|1 num2 high|low|0|1\n"
);
